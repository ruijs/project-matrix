...
// CRC16 lookup tables
const auchCRCHi = [0x00, 0xc1, 0x81, 0x40, 0x01, 0xc0, 0x80, 0x41, 0x01, 0xc0, 0x80, 0x41, 0x00, 0xc1, 0x81, 0x40];

const auchCRCLo = [0x00, 0xc0, 0xc1, 0x01, 0xc3, 0x03, 0x02, 0xc2, 0xc6, 0x06, 0x07, 0xc7, 0x05, 0xc5, 0xc4, 0x40];
...
export class TemperatureHexParser implements Parser {
  ...
  parse(payload: string | Buffer, clientId: string): ParserResult {
    ...
    try {
      const [pv, sp, heatHysteresis, coolHysteresis] = segments.map((segment) => this.parseDataSegment(segment));
